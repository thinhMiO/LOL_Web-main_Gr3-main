...
{   
	...
	$pdd=$_SESSION['pid'];
	$sql = "insert into orders(userId) values('".$_SESSION['id']."')";
	...
	foreach($value as $qty=> $val34){
		$sql = "SELECT * from products WHERE id =$qty";
		$pdo_stm = $conn->prepare($sql);
		$ketqua = $pdo_stm->execute();
		$row = $pdo_stm->fetch();
		$price =$row["productPrice"];
		
		mysqli_query($con,"insert into orders_detail(idorder,productId,quantity,price) values('$lastId','$qty','$val34','$price')");
...
?>
					...
					<td class="cart-product-sub-total"><span class="cart-sub-total-price" ><?php echo number_format($row['productPrice'],0,',','.')?><sup>đ</sup></span>


					<td class="cart-product-grand-total"><span class="cart-grand-total-price"><?php echo number_format($_SESSION['cart'][$row['id']]['quantity']*$row['productPrice']+$row['shippingCharge'],0,',','.'); ?><sup>đ</sup></span></td>
				</tr>
		...
		<thead>
			<tr>
				...
				<div  class="cart-grand-total black ">
						Tổng Tiền Hàng: <span class=""><?php echo number_format($_SESSION['tp']="$total",0,',','.')."<sup>đ</sup>"; ?></span>
					</div>
				<div class="cart-grand-total black">
					...
					<div class="cart-grand-total">
						Tổng Hóa Đơn:
						<span class=""><?php echo number_format($_SESSION['tp']="$totalprice",0,',','.'). "<sup>đ</sup>"; ?></span>
					</div>
				</th>
			</tr>
		</thead><!-- /thead -->
