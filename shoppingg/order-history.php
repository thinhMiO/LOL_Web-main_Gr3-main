		...
		<table class="table table-bordered">
			<thead>
				<tr>
					...
					<th class="cart-total item">Trạng thái đơn hàng</th>
...
<?php
$query=mysqli_query($con,"SELECT * from orders where orders.userId ='".$_SESSION['id']."' and orders.paymentMethod is not null order by orders.orderDate DESC " );
$cnt = 0;
...
?>
				<tr>
					<td><?php echo ++$cnt;?></td>
					<td><?=$row['idorder']?></td>
					<td class="cart-product-sub-total"><?php echo date("H:i d-m-Y",strtotime($row['orderDate'])); ?>  </td>
					...
					<td>
						<!-- <a href="javascript:void(0);" onClick="popUpWindow('track-order.php?oid=<?php echo htmlentities($row['idorder']);?>');" title="Track order">
						Xem</a> -->
						<a href="order-details.php?id=<?php echo htmlentities($row['idorder']);?>">Xem chi tiết đơn hàng</a>
					</td>
